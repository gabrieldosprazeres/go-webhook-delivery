...
for service in migrate api worker chaoslab; do
	compose build "$service"
done
compose up --detach --wait --no-build postgres api worker chaoslab
...
for service in api worker chaoslab migrate; do
