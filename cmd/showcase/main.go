...
<body>
  <header class="wrap nav"><div class="brand">webhook<span>.</span>engine</div><nav class="navlinks"><a href="#arquitetura">Arquitetura</a><a href="#engenharia">Engenharia</a><a class="button" href="{{.GitHubURL}}" rel="noopener noreferrer">Ver código ↗</a></nav></header>
  <main>
    <section class="wrap hero">
      <div><div class="eyebrow">Case técnico · Go + PostgreSQL</div><h1>Webhooks que chegam. Mesmo quando tudo falha.</h1><p>Uma engine de entrega durável com semântica <em>at-least-once</em>, retentativas inteligentes, isolamento multi-tenant e segurança aplicada desde a arquitetura.</p><div class="actions"><a class="button primary" href="{{.DocsURL}}" rel="noopener noreferrer">Explorar API no Swagger</a><a class="button" href="{{.GitHubURL}}" rel="noopener noreferrer">GitHub</a><a class="button" href="{{.ReleaseURL}}" rel="noopener noreferrer">Release</a></div></div>
