...
integration: ## Executa schema, concorrencia e E2E; exige as cinco WDE_TEST_*_DATABASE_URL.
	@test -n "$$WDE_TEST_API_DATABASE_URL" -a -n "$$WDE_TEST_WORKER_DATABASE_URL" -a -n "$$WDE_TEST_ADMIN_DATABASE_URL" -a -n "$$WDE_TEST_CONSOLE_DATABASE_URL" -a -n "$$WDE_TEST_SUPERUSER_DATABASE_URL"
...
build: ## Compila os seis binarios em bin/.
	mkdir -p bin
...
images: ## Constroi os cinco artefatos centrais de container com metadados OCI.
	WDE_VERSION=$${WDE_VERSION:-dev} WDE_REVISION=$${WDE_REVISION:-$$(git rev-parse HEAD)} \
		docker compose --profile demo build migrate api worker console chaoslab
...
supply-chain: ## Gera SBOM CycloneDX/SPDX e bloqueia High/Critical nas imagens exatas.
	...
	./scripts/supply-chain.sh "$$WDE_SUPPLY_CHAIN_OUTPUT" \
		webhook-delivery-engine-api webhook-delivery-engine-worker webhook-delivery-engine-console webhook-delivery-engine-chaoslab webhook-delivery-engine-migrate

container-smoke: ## Valida Compose e sobe os containers hardened em ambiente descartavel.
	./scripts/container-smoke.sh

rollback-rehearsal: ## Prova downgrade fail-closed com dados v6 e recuperacao forward.
